...
export function validateBearOffMove(player, fromPointId, diceValue, points) {
  const moveDistance = player === PLAYER.LEFT ? 25 - fromPointId : 13 - fromPointId;
...
export function findPotentialMoves(points, player, diceValue, checkersOnBar) {
  ...
  if (hasCheckerOnBar) {
    const startPointId = player === PLAYER.LEFT ? BOARD_CONFIG.START_KEY_LEFT : BOARD_CONFIG.START_KEY_RIGHT;
  ...
  for (const point of points.filter(p => p.player === player)) {
    for (const die of uniqueDice) {
      ...
      if (canBearOffNow) {
        ...
        if (isInHomeBoard) {
          const requiredDie = player === PLAYER.LEFT ? 25 - point.id : 13 - point.id;
