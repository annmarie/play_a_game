...
describe('Board Utilities', () => {
  describe('initializeBoard', () => {
    ...
    it('should initializes points with checkers and players', () => {
      const board = initializeBoard();
      expect(board[0]).toEqual({ id: 1, checkers: 5, player: PLAYER.LEFT });
      expect(board[11]).toEqual({ id: 12, checkers: 2, player: PLAYER.LEFT });
      expect(board[16]).toEqual({ id: 17, checkers: 3, player: PLAYER.LEFT });
      expect(board[18]).toEqual({ id: 19, checkers: 5, player: PLAYER.LEFT });
      expect(board[23]).toEqual({ id: 24, checkers: 2, player: PLAYER.RIGHT });
      expect(board[12]).toEqual({ id: 13, checkers: 5, player: PLAYER.RIGHT });
      expect(board[6]).toEqual({ id: 7, checkers: 5, player: PLAYER.RIGHT });
      expect(board[4]).toEqual({ id: 5, checkers: 3, player: PLAYER.RIGHT });
  ...
  describe('point to index map', () => {
    it('should return correct index point mapping for PLAYER_RIGHT', () => {
      const pointIdToIndexMap = getIndexToPointIdMap(PLAYER.RIGHT);
    ...
    it('should return correct index point mapping for PLAYER_LEFT', () => {
      const pointIdToIndexMap = getIndexToPointIdMap(PLAYER.LEFT);
    ...
    it('should return correct point index mapping for PLAYER_RIGHT', () => {
      const pointIdToIndexMap = getPointIdToIndexMap(PLAYER.RIGHT);
    ...
    it('should return correct point index mapping for PLAYER_LEFT', () => {
      const pointIdToIndexMap = getPointIdToIndexMap(PLAYER.LEFT);
      expect(pointIdToIndexMap[0]).toBe(11);
      expect(pointIdToIndexMap[11]).toBe(0);
      expect(pointIdToIndexMap[12]).toBe(12);
      expect(pointIdToIndexMap[23]).toBe(23);
